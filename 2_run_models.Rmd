---
title: "Step 2: Run spatiotemporal models for fisheries bycatch prediction"
author: "Brian Stock"
date: "July 3, 2018"
...
> Stock BC, Ward EJ, Eguchi T, Jannot JE, Thorson JT, Feist BE, and Semmens BX. "Comparing predictions of fisheries bycatch using multiple spatiotemporal species distribution model frameworks."
  ...
  * [GLM](#GLM)
  * [GAM CONSTANT (one spatial field)](#GAM_const)
  * [GAM IID (spatial field by year)](#GAM_iid)
  * [GMRF CONSTANT (one spatial field)](#GMRF_const)
  * [GMRF EXCHANGEABLE (spatial field by year)](#GMRF_iid)
  * [RF BASE](#RF_base)
  * [RF DOWN (downsample)](#RF_down)
  * [RF SMOTE (upsample + downsample)](#RF_smote)

We assume you either 1) have already seen [`1_process_survey`](https://rawgit.com/brianstock/spatial-bycatch/master/2a_process_survey.html), or 2) are not interested in how the data were processed/prepared. Either way, from this point we continue by using the saved output of `1_process_survey` (`wcann_processed.RData`).
...
library(mgcv)
library(INLA)
# INLA:::inla.dynload.workaround() # if old dependencies
```
...
###### 4. GMRF CONSTANT {#GMRF_const}
  
  > $Y$ ~ logDEPTH + logDEPTH$^2$ + sst + sst$^2$ + inRCA + DAY + YEAR + $MVN(0, Q^{-1})$
  
```{r}
# See below:
#   one GMRF function with input option to select CONSTANT vs. EXCHANGEABLE
```  
  
###### 5. GMRF EXCHANGEABLE {#GMRF_iid}
...
**Warning: this took 40 hours on a 12-core 32gb RAM machine**
